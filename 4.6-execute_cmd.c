...
char *find_command_path(const char *cmd)
{
	...
	else
	{
		...
		while (dir != NULL)
		{
			excutable = malloc(strlen(dir) + strlen(cmd) + 2);
			if (excutable == NULL)
			{
				free(excutable);
				return (NULL);
			}
			strcpy(excutable, dir);
			strcat(excutable, "/");
			strcat(excutable, (char *) cmd);
...
int excutcmd(char **cmd)
{
	...
	{
		comp = strncmp(cmd[0], builtin_cmds[i].name,
				strlen(builtin_cmds[i].name));
		if (comp == 0)
		{
